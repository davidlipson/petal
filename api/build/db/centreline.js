...
exports.centreline = void 0;
const centreline = (pool, start_address, end_address) => __awaiter(void 0, void 0, void 0, function* () {
    const results = yield pool.query(`with startadd as (select *, 'START-TERMINUS' node_name from address a where (concat(address, ' ', lfname) = '${start_address}')  limit 1),
...
select road_type, st_asgeojson(a_geometry) a_geometry, st_asgeojson(b_geometry) b_geometry, street_name, departing_angle, arriving_angle, st_asgeojson(geometry) geometry,  street_length, a_name, b_name from finalUnion`);
    return results.rows;
});
